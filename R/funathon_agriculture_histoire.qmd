...
mon_canton <- surfaces_ra1852 %>% 
  filter(canton == "DOUAI") %>% ### Changer ici avec votre canton
  pivot_longer(-c(code, canton, departement)) %>% 
  select(-c(code, canton, departement)) %>% 
  print()
```
```{r}
verif<-surfaces_ra1852 %>% 
  mutate(test_TL=total_des_terres_labourables - jacheres - prairies_artificielles -cultures_diverses -racines_et_legumes_divers - cereales)
```
...
curl "https://geo.api.gouv.fr/communes?nom=Douai&codeDepartement=59"
...
curl "https://geo.api.gouv.fr/communes?nom=Douai&codeDepartement=59&fields=centre&boost=population&limit=1"
```

On obtient bien les coordonnées du centre de Douai : 
[3.1004,50.3785]

Pour vérifier on peut utiliser google maps, en mettant directement les coordonnées dans l'url après le @ : 
[https://www.google.fr/maps/@43.6007,1.4328,13.5z](https://www.google.fr/maps/@43.1004,50.3785,13.5z) (le 13.5z est le zoom)
...
url_completed <- sprintf(url, "Lille", "59")
...
#| warning: false
#| echo: true
tab_dept<-surfaces_ra1852 %>% filter(is.na(departement)==TRUE) %>% 
 mutate(codeNew1=code/10,
  codeNew2=as.character(codeNew1),
  codeNew3=str_pad(codeNew2,2,pad="0",side="left")) %>%  select(code_dept=codeNew3,lib_dept=canton)

surfaces_ra1852_bis <- surfaces_ra1852 %>% filter(is.na(departement)==FALSE) %>% 
left_join(tab_dept,by=c("departement"="lib_dept")) %>% mutate(canton2=str_replace_all(surfaces_ra1852_bis$canton," ","-"))
...
trouver_centre <- function(url,lib_canton, code_dep){
  url_completed <- sprintf(url,lib_canton, code_dep)
  resultat <-  fromJSON(url_completed)
...
surfaces_ra1852_coordonnees_centre <- surfaces_ra1852_bis %>%
  rowwise() %>%
